...
import { applyWebhookToOrder, verifyWebhookSignature } from "@/lib/shiprocket";

// Shared Shiprocket tracking-webhook handler. Exposed at two routes:
//   /api/shiprocket/webhook   (original)
//   /api/delivery-webhook     (keyword-free — Shiprocket rejects URLs that
//                              contain "shiprocket"/"sr"/"kr")
// Header "X-Api-Key" must contain the secret saved in admin Shipping settings.
export async function handleTrackingWebhook(req: Request) {
  try {
